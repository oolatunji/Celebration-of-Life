...
function displayExistingContentIfAny() {
    if (window.sessionStorage.getItem('webContent') != null) {

        var webContent = JSON.parse(window.sessionStorage.getItem('webContent'));

        var categoryName = webContent.Name;
        var categoryType = webContent.Type;
        var categoryId = webContent.Id;
        var categoryContent = webContent.Content;

        $('#contentCategory').val(categoryId);
        $("#contentCategory").attr("disabled", "disabled");
        ...
        if (_.isEqual(categoryType, 'Text')) {
            ...
            $("#txtEditor").Editor("setText", categoryContent);
            $("#buttons").show();
            ...
            var count = 0;
            names = [];
            $.each(categoryContent, function (key, image) {
                ...
                removeImageLoadedIcon.onclick = function () {
                    var divToRemove = document.getElementById(count);
                    div.parentNode.removeChild(div);
                    names.splice(names.indexOf(count), 1);
                };
    ...
    if (_.isEmpty(catID)) {
        ...
        if (_.isEqual(categoryType, 'Text')) {
            ...
            if (_.isEmpty(webContentText)) {
                ...
                $('#publishBtn').html('<i class="fa fa-spinner fa-spin"></i> Updating Content...');
                $("#publishBtn").attr("disabled", "disabled");

                $.ajax({
                    url: settingsManager.websiteURL + 'api/ContentAPI/UpdateContent',
                    type: 'PUT',
                    data: webContent,
                    processData: true,
                    async: true,
                    ...
                    success: function (response) {
                        ...
                        $('#publishBtn').html('<i class="fa fa-video-camera"></i> Update Content');

                        window.sessionStorage.setItem('msg', response)
